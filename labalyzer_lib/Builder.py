...
# from gi.repository import GObject  #EDIT:WINDOWS # pylint: disable=E0611
import gobject #EDIT:WINDOWS
...
def auto_connect_by_name(callback_obj, builder):
	...
	for item in builder.widgets.items():
		...
		try:
			...
			while widget_type:
				signal_ids.extend(gobject.signal_list_ids(widget_type)) #EDIT:WINDOWS GObject to gobject
				widget_type = gobject.type_parent(widget_type) #EDIT:WINDOWS GObject to gobject
		except RuntimeError:  # pylint wants a specific error
			pass
		signal_names = [gobject.signal_name(sid) for sid in signal_ids] #EDIT:WINDOWS GObject to gobject

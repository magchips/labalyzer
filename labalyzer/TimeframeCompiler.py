...
class TimeframeCompiler:
    ...
    def compileTimeframe(self, timeframe):
        ...
        trigBoard2 = settings['DigitalChannels']['AnalogTrigger2'].portNumber                           # needed multiple times below
        trigMask2 = settings['DigitalChannels']['AnalogTrigger2'].bitmask                           # needed multiple times below
        invertedTrigMask2 = settings['DigitalChannels']['AnalogTrigger2'].invertedBitmask
        ...
        for row in timeframe:                                               # go through the timeframe row by row
            ...
            if rowtype == ROWTYPE_ANALOG:
                ...
                if rampType == 'step':  # so we just add one command
                    if not abs_time in analogCommands: # need to add triggers to the digital commands
                                                if boardNumber == 1:
                                                        ...
                                                        digitalCommands[abs_time].append((trigBoard2, trigMask2, 1))
                                                        digitalCommands[abs_time + TRIGGER_LENGTH].append((trigBoard2, invertedTrigMask2, 0))
                        ...
                        for i, t in enumerate(times):
                            if not t in analogCommands: # need to add triggers
                                                                if boardNumber == 1:
                                                                        ...
                                                                        digitalCommands[t].append((trigBoard2, trigMask2, 1))
                                                                        digitalCommands[t + TRIGGER_LENGTH].append((trigBoard2, invertedTrigMask2, 0))
                ...
                if value == settings['DigitalChannels'][device].highname:
                                        if settings['AnalogChannels'][device].boardNumber == 1:
                                                digitalCommands[abs_time].append((settings['DigitalChannels'][device].portNumber, settings['DigitalChannels'][device].bitmask, 1))
                                        elif settings['AnalogChannels'][device].boardNumber == 2:
                                                digitalCommands[abs_time].append((settings['DigitalChannels'][device].portNumber, settings['DigitalChannels'][device].bitmask2, 1))
                                        ...
                                        if settings['AnalogChannels'][device].boardNumber == 1:
                                                ...
                                                digitalCommands[abs_time].append((settings['DigitalChannels'][device].portNumber, settings['DigitalChannels'][device].invertedBitmask2, 0))
        ...
        if settings['AnalogChannels'][device].boardNumber == 2:
                        digitalCommands[abs_time+2*TRIGGER_LENGTH] = [(trigBoard2, trigMask2, 1)]
                        digitalCommands[abs_time+3*TRIGGER_LENGTH] = [(trigBoard2, invertedTrigMask2, 0)]
                        totalTime = abs_time+3*TRIGGER_LENGTH
        elif settings['AnalogChannels'][device].boardNumber == 1:
        ...
        AOSamples = [numpy.zeros((len(analogCommands)*8, ), dtype=numpy.float64 ),
                             numpy.zeros((len(analogCommands)*8, ), dtype=numpy.float64 ),
        ...
        for i, k in enumerate(analogKeys):
            if i != 0:
                ...
                AOSamples[2][8*i:8*(i+1)] = AOSamples[2][8*(i-1):8*i]
